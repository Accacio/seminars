...
\date
{\textbf{Seminar \\École Centrale de Lyon / Laboratoire Ampère}
  \\
  25/05/2023 @ Écully\\
  ...
  \begin{minipage}{.5\textwidth}
    \centering
    \vspace{10pt}
    \qrcode[height=1.5cm]{https://github.com/Accacio/seminars/raw/main/2023-ECL_Ampere/presentation/presentation.pdf}

    \vspace{5pt}
    \href{https://bit.ly/43h2jms}{https://bit.ly/43h2jms}
  \end{minipage}
  % \begin{minipage}{.3\textwidth}
  %   \centering
  %   \vspace{10pt}
...
\begin{frame}{About me}
  ...
  \begin{minipage}[c]{.7\linewidth}
    \begin{itemize}
      \item[] Postdoctoral researcher at LAAS/CNRS
      \item[] \textit{Guaranteed relative localization and anti collision scenario for autonomous vehicles}
      \item[] Project AutOCampus (GIS neOCampus)
      \item[] Advised by Soheib Fergani
    \end{itemize}
  \end{minipage}
  \hfill
  ...
  \begin{minipage}[c]{.7\linewidth}
    \begin{itemize}
      \item[] Bachelor Thesis at Escola Politécnica/UFRJ
      \item[] \textit{Identification of DES for fault-diagnosis}
      \item[] Advised by Marcos Vicente de Brito Moreira
    \end{itemize}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{.2\linewidth}
    ...
    \includegraphics[width=3cm]{logos/ufrj.jpeg}%
  \end{minipage}

  % \centering
  \begin{minipage}[c]{.4\textwidth}
  \includegraphics[width=1.5cm]{maquete/mag.jpg}
  \includegraphics[width=1.5cm]{maquete/esteira.jpg}
  \includegraphics[width=1.5cm]{maquete/sensores.jpg}

  \includegraphics[width=1.5cm]{maquete/braco.jpg}
  \includegraphics[width=1.5cm]{maquete/prensa.jpg}
  \includegraphics[width=1.5cm]{maquete/elevador.jpg}
  \end{minipage}
  \tikz\draw[->] (0,0) - +(2,0) node [midway,above,align=center] {I/O\\ +\\ DAOCT};
  \begin{minipage}[c]{.4\textwidth}
  \includegraphics[width=6cm]{maquete/example.pdf}
  \end{minipage}
  }
  ...
  \begin{minipage}[c]{.7\linewidth}
    \begin{itemize}
      \item[] Doctoral Thesis at CentraleSupélec/IETR
      \item[] \textit{Security of dMPC under False Data Injection}
      \item[] Advised by Hervé Guéguen and Romain Bourdais
    \end{itemize}
    \begin{itemize}
      \item[] CS-Rennes  (Expertise in MPC for Smart Buildings)
      \item[] Brittany Region Interest (Cybersecurity)
    \end{itemize}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{.2\linewidth}
...
\begin{frame}{Context}{Smart(er) Cities}
        ...
        \begin{itemize}
          ...
          \item<9-> We also want
                \begin{itemize}
                  ...
                  \item<11-> Maximizer satisfaction
                  \item<12-> Follow a trajectory
                \end{itemize}
        \end{itemize}
        \begin{itemize}
          \item<13-> Solution $\to$ MPC
        \end{itemize}
      \end{minipage}
    }%

    % \onslide<14->{\tikz\node[draw,rectangle,white,inner sep=7.5pt,text=black,fill=mpc_coordinator]{How to design safe dMPC};}
  }
  \end{overlayarea}
\end{frame}

\begin{frame}{Model-based Predictive Control}{Brief recap}
  \pause
  Find \tikz[baseline=(best.base)]\node[minimum width=8ex,inner sep=0pt] (best) {\only<1-3|handout:0>{\alert<3>{best}}\only<4->{optimal}}; control sequence using predictions based on a model.
  \begin{itemize}
    \item<5-> We need an optimization problem
          \begin{itemize}
            \item<6-> Decision variable is the control sequence \only<7->{calculated over horizon N}
            \item<8-> Objective function to optimize
            \item<9-> System's Model
            \item<10-> Other constraints to respect \onslide<11->{(QoS, technical restrictions, ...)}
          \end{itemize}
  \end{itemize}
  \vspace{1cm}
  % \only<4-12>{
  \visible<5->{
    \begin{equation*}
      \begin{matrix}
        \minimize\limits_{\alert<6|handout:0>{\vec{u}[0:\alert<7|handout:0>{\predhorz}-1|k]}} & \alert<8|handout:0>{J(\vec{x}[0|k],\vec{u}[0:\alert<7|handout:0>{\predhorz}-1|k])}&\\
        \onslide<9->{\mathrm{subject~ to} &
                                            \left.
                                            \begin{matrix}
                                              \alert<9|handout:0>{\vec{x}[\xi|k]=f(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])}\\
                                              \onslide<10-11>{\alert<10|handout:0>{g_{i}(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])\leq0}}\\
                                              \onslide<10-11>{\alert<10|handout:0>{h_{j}(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])=0}}\\
                                            \end{matrix}
        \right\}
        \begin{aligned}
          &\forall \xi\in\{1,\dots,\alert<5|handout:0>{\predhorz}\}\\
          &\onslide<10-11>{\alert<10|handout:0>{\forall i\in\{1,\dots,m\}}}\\
          &\onslide<10-11>{\alert<10|handout:0>{\forall j\in\{1,\dots,p\}}}
        \end{aligned}}
      \end{matrix}
    \end{equation*}
    % }
  ...
  \script<9>{with its states }
  \script<9>{and inputs }
  \script<10>{But we can also integrate some constraints, such QoS or technical restrictions}
  ...
  \begin{overlayarea}{\textwidth}{\textwidth}
    ...
    \begin{figure}
      ...
      \foreach \i in {1,...,6}{%
        ...
        \only<{\i}|handout:{\i}>{\input{../img/mpcState\i.tex}}%
        \only<{\i}|handout:{\i}>{\input{../img/mpcControl\i.tex}}%
      }
    \end{figure}
  \end{overlayarea}
...
\begin{frame}{Model Predictive Control}{Nothing is perfect}
  ...
  \begin{minipage}[t]{.4\linewidth}
    \begin{itemize}[<+(1)->]
      \item Solution: distributed MPC
    \end{itemize}
  \end{minipage}

\end{frame}

\begin{frame}{Objective}{Study security in dMPC context}
  \onslide<+(1)->{
    Security in dMPC context is relatively new\footnote<.(1)->{<30 documents in scopus} (First article from 2017\footnote<.(1)->{\cite{VelardeEtAl2017b}})
  }

  \vspace{.5cm}
  \onslide<+(1)->{\tikz\node[draw,rectangle,white,inner sep=5.5pt,text=black,fill=mpc_coordinator]{
      \vbox{
      \begin{itemize}
        \item How fragile are dMPC structures?
        \item How can agents act non-cooperatively?
        \item How to identify such agents and mitigate the effects?
      \end{itemize}
      }
    };}
...
\begin{frame}{Outline}
  % \tableofcontents
  \tableofcontents[subsectionstyle=hide/hide/hide,subsubsectionstyle=hide,pausesections]
...
\begin{frame}{Distributed Model Predictive Control}
  \begin{itemize}
    \item We break the MPC optimization problem
    \item Make agents communicate
  \end{itemize}

  \onslide<+(1)->{In other words}
  \begin{itemize}[<+(1)->]
    \item Agents solve local problems\tikzmark{a}
    \item Exchange some variables
    \item Variables are updated\tikzmark{ab}
  \end{itemize}
  \onslide<+(1)->{
    \begin{tikzpicture}[overlay, remember picture,decoration={brace,amplitude=2pt}]
      \draw[decorate,thick] (a.north east) -- (a.north east |- ab.south east) node[midway, right=0.1cm,text=black,text width = 2in] {Until\\ Convergence};
    \end{tikzpicture}%
  }

  \onslide<+(1)->{%
    \begin{remark}
      If agents exchange same variable $\to$ consensus problem
    \end{remark}
  }
...
Usually based on optimization decomposition methods\footnote{\articlesymbol~\cite{BoydEtAl2015}}:

\begin{itemize}[<+(1)->]
  \item \alert<.(3)-.(5)|handout:0>{Local problems with auxiliary variables}
  \item \alert<.(5)-.(6)|handout:0>{Update auxiliary variables}
\end{itemize}
\onslide<+(1)->{Basically 2 choices\footnote<.(1)->{Other approaches, but similar concepts}:}
  \begin{itemize}[<+(1)->]
    \item Modify based on dual problem\footnote<.(1)->{Lagrangian, ADMM, prices, etc +1000 articles in scopus}  (Solve with dual and send primal)
    \item Modify based on \alert<.(4)->{\tikz[baseline={(a.base)},remember picture]\node[inner sep=0pt] (a) {primal problem};} (Solve with primal and send dual)
  \end{itemize}

\onslide<+(1)->{Many methods:}
  \begin{itemize}[<+(1)->]
    \item Cutting plane, \alert<.(2)->{\tikzmarktext{b}{sub-gradient}} methods, \dots
  \end{itemize}

  \onslide<+(2)->{
    \begin{tikzpicture}[overlay, remember picture,decoration={brace,amplitude=2pt}]
      \draw[->,thick] (a.south) to[out=-30,in=180] +(2,-.5) node[right,text=black,text width = 2in] (text) {Security/privacy properties};
      \draw[->,thick] (b.north) to[out=30,in=180] (text.west);
    \end{tikzpicture}%
  }
...
\begin{frame}{Distributed Model Predictive Control}{It is about communication}
  ...
  \begin{itemize}
    ...
    \item<+-> Make agents communicate. \only<+->{ But how?}
          \begin{itemize}
            \item<+-> Many flavors to choose from\visible<.(5)->{\footnote<.(5)->{{{\booksymbol~\cite{MaestreEtAl2014}}}}}
                  % \visible<.(5)->{\footnote<.(5)->{{{\booksymbol~\cite{Nogueira2022}}}}}
                  \begin{itemize}[<+->]
                    \item \alert<.(5)>{Hierarchical}/Anarchical
                    \item \alert<.(4)>{Parallel}/Sequential
...
let \p1=(\tikztostart), \p2=(\tikztotarget), \n1={veclen(\y2-\y1,\x2-\x1)} in
  ...
  %   \node[] at (-2,3) {\includegraphics<7>[width=.35\textwidth]{nazare/12.png}};
    ...
    \node[] at (-1.5,3) {\includegraphics<7->[width=.3\textwidth]{do_not_panic.png}};
  \end{tikzpicture}%
  \script<1>{However, the solution will depend on the horizon, the number of constraints, and sizes of input and states, increasing the complexity of the calculation }
  \script<2>{A strategy to alleviate is to distribute the calculation whenever possible. And there are many ways to divide it as the book shows.}
  \script<3>{Here we opt for a hierarchical strategy where we use multiple MPCs and an agent to coordinate and manage the coupling aspects of the problem.}
...
\begin{frame}{Distributed Model Predictive Control}{Optimization Decomposition}
  ...
  \begin{minipage}[c]{.5\linewidth}
    \begin{itemize}[<+(2)-|handout:+(2)->]
      \item Coordinator \only<.(4)-|handout:.(4)->{$\to$ Hierarchical}
            \begin{itemize}[<+(2)-|handout:+(2)->]
              \item Enforce global constraints
            \end{itemize}
      \item<+(3)-|handout:+(3)-> Bidirectional
      \item<+(3)-|handout:+(3)-> No delay $\to$ Synchronous
      \item<+(3)-|handout:+(3)-> But what to send?
    \end{itemize}
    \vspace{.5cm}
    \centering
  \end{minipage}
...
\begin{frame}{Primal Decomposition}{or Quantity Decomposition | or Resource Allocation}
  ...
  \begin{minipage}{.45\linewidth}
    \begin{figure}
      ...
      \scalebox{.55}{
        \begin{tikzpicture}[font=\small,thick,node distance=.6cm and 6cm,
          ...
          \node[terminal, alt=<{2}|handout:{2}>{selected}] (block1) {Start};

          \node[action, alt=<{3}|handout:{3}>{selected},below=of block1,] (block7) { Coordinator sends \\$\vec{\theta}_i^{(h)}$};

          \node[action, alt=<{4}|handout:{4}>{selected}, below=of block7,] (block8) { Agents send \\$\vec{\lambda}_i(\vec{\theta}_{i}^{(h)})$};

          \node[action, alt=<{5}|handout:{5}>{selected}, below=of block8,
          ] (block9) { Coordinator updates \\allocation};

          \node[condition, alt=<{6}|handout:{6}>{selected}, below=of block9] (block10) { Negotiation\\ converges?};

          \node[draw,
          rounded rectangle,
          below=of block10,
          fill=gray!90!black!50,
          alt=<{7}|handout:{7}>{selected},
    ...
    \end{figure}
  \end{minipage}
  \only<2-|handout:2->{\begin{minipage}{.45\linewidth}
      \begin{figure}
        \centering
        \begin{tikzpicture}[balloon/.style={
          ...
          \begin{scope}
            \clip[rounded corners] (0,0) rectangle (5,3.5) node[midway] (image) {\includegraphics[width=1.\textwidth,trim=2cm 2 20 4,clip]{cut-cake.jpg}};
          \end{scope}
      ...
      \end{figure}
    \end{minipage}}
  \script<1-|handout:1->{In a flowchart for a quantity decomposition based DMPC,}
  \script<3-|handout:3->{the coordinator sends the allocation theta, }
  \script<4-|handout:4->{the agents send the dual variable lambda, }
  \script<5-|handout:5->{the coordinator updates the allocation. }
  \script<6-|handout:6->{If negotiation converges, }
  \script<7-|handout:7->{then the negotiation ends and each agent applies the first element of the control found}
\end{frame}

\begin{frame}{Primal Decomposition }{In detail}
  \begin{minipage}[c]{.45\linewidth}
    \begin{itemize}[<+(1)-|handout:0>]
      \item Objective is sum of local ones
      \item Constraints couple variables
    \end{itemize}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{.45\linewidth}
    \onslide<1-4>{
      \begin{equation*}
        \begin{matrix}
          \minimize\limits_{\vec{u}_{1},\dots,\vec{u}_{M}} & \alert<2|handout:0>{\sum\limits_{i\in\set{M}}J_{i}(\vec{x}_{i},\vec{u}_{i})}\\
          \alert<3|handout:0>{\mathrm{s.t.}} &\alert<3|handout:0>{\sum\limits_{i\in\set{M}}\vec{h}_{i}(\vec{x}_{i},\vec{u}_{i})\preceq \vec{u}_{\text{total}}}
        \end{matrix}
      \end{equation*}
    }
  \end{minipage}
  ...
  \begin{minipage}[c]{.5\linewidth}
    \begin{enumerate}[<+(2)->]
      ...
      \item Allocation is updated\footnotemark \onslide<9->{\\(respect global constraint)}
    \end{enumerate}
  \end{minipage}
  \addtocounter{footnote}{-1}%
  \footnotetext<.(1)->{It obfuscates system's parameters (+ Privacy)}\addtocounter{footnote}{1}
...
\begin{minipage}[c]{.4\linewidth}
  ...
  \begin{minipage}[c]{.5\linewidth}
    \begin{itemize}[<+(1)->]
      \item  Common attacks\footnotemark
            \begin{itemize}[<.(3)->]
              \item Fake objective function\tikzmark{a}
              \item Fake constraints
              \item Use different control\tikzmark{ab}
            \end{itemize}
    \end{itemize}
  \end{minipage}
  \footnotetext<.(1)->{\cite{VelardeEtAl2017a}}
  \onslide<+(3)->{
    \begin{tikzpicture}[overlay, remember picture,decoration={brace,amplitude=2pt}]
      \draw[decorate,thick] (a.north east) -- (a.north east |- ab.south east) node[midway, right=0.1cm,text=black,text width = 2in] {Deception Attacks};
...
\begin{frame}{How can a non-cooperative agent attack?}{Our approach\footnote{\cite{NogueiraEtAl2021}}}
\begin{minipage}[c]{.4\linewidth}
    \begin{figure}[H]
      \centering
      \begin{tikzpicture}[font=\small,thick,node distance=3*0.6180cm and 0.6180cm,every node/.style=rectangle,
...
\begin{frame}{How can a non-cooperative agent attack?}{Our approach\footnote{\cite{NogueiraEtAl2021}}}
    ...
    \begin{figure}[H]
      \centering
        \begin{tikzpicture}[font=\small,thick,node distance=3*0.6180cm and .618cm,every node/.style=rectangle,
          mpcSmall/.style={fill=mpc_agent, minimum height=0.6180*2cm, minimum width=2cm,text=white},
          coordinator/.style={fill=mpc_coordinator, minimum height=0.6180*3cm, minimum width=6cm},
          ]
          ...
          \only<-2|handout:0>{\draw[-latex,line width=1pt] (block1.south)+(0.4,.0) -- ( coordinator.north -| {$(block1.south)+(0.4,.0)$}) node (lambda1) [right,midway] {$\alert<2>{\lambda_{1}}$};}
          \only<3->{\draw[-latex,line width=1pt] (block1.south)+(0.4,.0) -- ( coordinator.north -| {$(block1.south)+(0.4,.0)$}) node (lambda1) [right,midway] {$\alert<3->{\tilde{\lambda}_{1}}$};}
    ...
    \begin{minipage}[c]{.5\linewidth}
      ...
      \begin{itemize}
        ...
        \item<2-> Malicious agent modifies $\vec{\lambda}_{i}$
      \end{itemize}
      \centering
      \only<3->{$\tilde{\vec{\lambda}}_{i}=\gamma_{i}(\vec{\lambda}_{i})$\\}
    \end{overlayarea}
    \end{minipage}
  ...
  \begin{minipage}[c]{.4\linewidth}
    ...
    \onslide<+(1)->{
      \begin{tikzpicture}[domain=0:2]
        ...
        \only<.(2)->{\draw[red] (0,0) circle (0.1);}

        \only<.(2)->{\draw[color=gray,dashed,domain=0:4] plot (\x,\x);}
        ...
        \only<.(3)->{\draw[color=blue,domain=0:1.25] plot (\x,3*\x) node[above,xshift=.9cm] {$a\lambda\text{, }a>1$} ;}
      \end{tikzpicture}
    }
  \end{minipage}
\end{frame}
    ...
    \onslide<2->{
      \begin{exampleblock}{4 distinct agents}
        \begin{itemize}
          ...
          \item Simulate for different $\tau_{1}$ get $J_{i}$
        \end{itemize}
      \end{exampleblock}
    }
    \begin{itemize}[<+(3)->]
...
\begin{frame}{Our Approach}{Explore Scarcity}
  \begin{itemize}
    \item Resilient
    \item Analytical/Learning\tikzmark{a}
    \item Data reconstruction\tikzmark{ab}
  \end{itemize}

  \onslide<+(1)->{
    \begin{tikzpicture}[overlay, remember picture,decoration={brace,amplitude=2pt}]
      \draw[decorate,thick] (a.north east) -- (a.north east |- ab.south east) node[midway, right=0.1cm,text=black,text width = 2in] {Parameter\\ Estimation};
    \end{tikzpicture}%
  }
...
\begin{frame}{Outline}
  \tableofcontents[sectionstyle=show/hide,subsectionstyle=show/hide/hide,subsubsectionstyle=show/show/hide/hide]
\end{frame}

\subsubsection{Resilient Primal Decomposition-based dMPC for deprived systems}
% \subsubsection{Analyzing deprived systems}
% \subsubsection{Building an algorithm}
% \subsubsection{Applying mechanism}
...
\begin{frame}{Complete algorithm}{RPdMPC-DS\footcite{NogueiraEtAl2021}}
  \begin{figure}[h]
    \centering
    \scalebox{.45}{
      \begin{tikzpicture}[font=\small,thick,node distance=.6cm and 6cm]
  ...
  \only<+(1)->{
    \begin{minipage}[t]{.50\linewidth}
      ...
      \only<.(1)->{
        ...
        \begin{tikzpicture}
          \begin{scope}
            \clip[rounded corners] (0,0) rectangle (3.,2.25) node[midway] (image) {\reflectbox{\includegraphics[width=.5\textwidth,trim=0 .3cm 0 .2cm,clip]{../img/mutley.jpg}}};
          \end{scope}
        \end{tikzpicture}
      }
...
\subsubsection{Resilient Primal Decomposition-based dMPC using Artificial Scarcity}
\begin{frame}{Outline}
  \tableofcontents[sectionstyle=show/hide,subsectionstyle=show/hide/hide,subsubsectionstyle=show/hide/hide/hide]
\end{frame}

% \subsubsection{Relaxing some assumptions}
% \subsubsection{Adapting the algorithm}
% \subsubsection{Applying mechanism}
...
\begin{frame}{Complete algorithm}{RPdMPC-AS\footcite{NogueiraEtAl2022}}
  \begin{figure}[h]
    \centering
  \scalebox{.45}{
    \begin{tikzpicture}[font=\small,thick,node distance=.6cm and 6cm]
  ...
  \begin{minipage}[c]{.6\textwidth}
    \begin{exampleblock}{District Heating Network (4 Houses)}
      \begin{itemize}
        ...
        \item \only<1|handout:0>{Not enough power}\only<2->{\st{Not enough power} (Change $(\vec{x}_{0},\vec{w}_{0})$)}
        \item Period of ${5} \mathrm{h}$ ($T_{s}=0.25h$ $\to k=\{1\mathbin{:}20\}$)
        \item Prediction horizon ($\predhorz =4$)
        \item 3 scenarios
              \begin{itemize}
...
\begin{frame}{Results}{Temporal}
  ...
  \only<2->{
    \begin{tikzpicture}[overlay, remember picture]
      % \clip[rounded corners] ($(12.,4.5)-(3.,2.25)$) rectangle +(3.,2.25)
      \clip[rounded corners] ($(12.,4.5)-(1.5,1.125)$) rectangle +(3.,2.25)
      node at (12.,4.5){
...
\begin{frame}[plain]
  ...
  \begin{minipage}[t]{.45\linewidth}
    ...
    \vspace{5pt}
    {\qrcode[link, height=1.5cm]{https://github.com/Accacio/thesis}}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{.5\linewidth}
    \small
    ...
    \vspace{5pt}
    {\qrcode[link, height=1.5cm]{mailto:rafael.accacio.nogueira@gmail.com?subject=Seminar\ Lyon\ 2023}}
  \end{minipage}
