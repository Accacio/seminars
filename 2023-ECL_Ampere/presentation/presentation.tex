...
\begin{itemize}[<+(1)->]
  \item \alert<.(3)-.(5)>{Local problems with auxiliary variables}
  \item \alert<.(5)-.(6)>{Update auxiliary variables}
\end{itemize}
...
\begin{frame}{Distributed Model Predictive Control}{Optimization Decomposition}
  ...
  \begin{minipage}[c]{.5\linewidth}
    \begin{itemize}[<+(2)->]
      \item Coordinator \only<.(4)->{$\to$ Hierarchical}
            \begin{itemize}[<+(2)->]
              \item Enforce global constraints
            \end{itemize}
      \item<+(3)-> Bidirectional
      \item<+(3)-> No delay $\to$ Synchronous
    \end{itemize}
    \vspace{.5cm}
    \centering
  \end{minipage}
