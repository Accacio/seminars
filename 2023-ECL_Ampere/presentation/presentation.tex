...
\begin{frame}{Context}{Smart(er) Cities}
  ...
  \centering
  Multiple systems interacting \visible<8->{under}
...
\begin{frame}{Model-based Predictive Control}{Brief recap}
  \pause
  Find \tikz[baseline=(best.base)]\node[minimum width=8ex,inner sep=0pt] (best) {\only<1-3>{\alert<3>{best}}\only<4->{optimal}}; control sequence using predictions based on a model.
  \begin{itemize}
    ...
    \begin{equation*}
      \begin{matrix}
        ...
        \onslide<9->{\mathrm{subject~ to} &
                                            \left.
                                            \begin{matrix}
                                              \alert<9>{\vec{x}[\xi|k]=f(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])}\\
                                              \onslide<10-11>{\alert<10>{g_{i}(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])\leq0}}\\
                                              \onslide<10-11>{\alert<10>{h_{j}(\vec{x}[\xi-1|k],\vec{u}[\xi-1|k])=0}}\\
                                            \end{matrix}
        \right\}
        \begin{aligned}
          &\forall \xi\in\{1,\dots,\alert<5>{\predhorz}\}\\
          &\onslide<10-11>{\alert<10>{\forall i\in\{1,\dots,m\}}}\\
          &\onslide<10-11>{\alert<10>{\forall j\in\{1,\dots,p\}}}
        \end{aligned}}
      \end{matrix}
    \end{equation*}
    % }
...
\begin{frame}{Primal Decomposition }{In detail}
  ...
  \begin{minipage}[c]{.5\linewidth}
    \begin{enumerate}
      ...
      \item<8-> Allocation is updated \onslide<9->{\\(respect global constraint)}
    \end{enumerate}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{.45\linewidth}
...
\begin{frame}{Distributed Model Predictive Control}
  ...
  Problem recent in dMPC literature\footnote<.(1)->{<30 documents in scopus} (First article from 2017\footnote<.(1)->{\cite{VelardeEtAl2017b}})\pause

  \vspace{.25cm}
  \begin{itemize}
    \item CentraleSupélec Rennes - Expertise in MPC for Smart Buildings\pause
    \item Incentive Brittany Region (Sustainable Energy + cybersecurity)\pause
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Can we mitigate the effects? How?
  \end{itemize}
\end{frame}
...
\begin{frame}{How can a non-cooperative agent attack?}{Our approach\footnote{\cite{NogueiraEtAl2021}}}
    ...
    \begin{minipage}[c]{.5\linewidth}
      ...
      \begin{itemize}
        ...
        \item<2-> $\vec{\lambda}_{i}$ obfuscate params. (+ Privacy)
...
%       \item ...
%       \item Scalability with multiple hierarchical layers
%       \item Uncertain large scales
%       \item Other topologies (does it increase/decrease vulnerabilites?)
%     \end{itemize}
% \end{frame}
